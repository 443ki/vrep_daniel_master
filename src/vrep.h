...
/**
 * @file vrep.h
 * @brief V-REPを使うためのクラス 亀川先生が好きそうなコーディングに変更
 * @author Akiyama Taichi
 * @date 2018/11/06
 * @detail
 */

#ifndef ROBOT_CONTROL_VREP_SRC_VREP_H_
#define ROBOT_CONTROL_VREP_SRC_VREP_H_
...
class Vrep {
 ...
 public:
  ...
  void WaitForReachingLastCommand();
 ...
 private:
  ...
  bool is_connecting_to_vrep_;
  bool is_sim_running_;
  bool is_sim_pausing_;
...
/*
V-REPのRemotoAPIの仕様で何か命令を出した際にその結果（エラーの有無、絵アラーの内容）を
returnするようになっている。
ReturnCordのクラスはその実行結果を格納するクラスで、RemotoAPIを使用するたびに作成されるクラスになっている。
各種エラーの詳細は公式を参照
*/
...
class Vrep::VisionSensor : public Vrep::Object {
  public:
    typedef std::vector<VisionSensor>::iterator ItrType;
    VisionSensor() {
      Object();
    }
    ~VisionSensor() {
    }
    VisionSensor(const VisionSensor& obj){ // コピーコンストラクタ
    }
    void SetPerspectiveAngle(double angle, bool unit_is_deg);

  private:
};

#endif /* ROBOT_CONTROL_VREP_SRC_VREP_H_ */
