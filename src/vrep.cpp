...
bool Vrep::Object::GetHandle(char* object_name) {
  name_ = object_name;
  ReturnCode ret_code(
      simxGetObjectHandle(Vrep::client_id_, (simxChar*) object_name,
...
/** @fn
 * @brief カメラの視野角を設定
 * @param (double) angle [rad](or [deg])
 * @param bool unit_is_deg angleの角度が[deg]の時trueにする
 * @return なし
 * @detail　2019/12/06追加
 */
void Vrep::VisionSensor::SetPerspectiveAngle(double angle, bool unit_is_deg = false){
  if (unit_is_deg) { angle *= M_PI/180.0; }
  simxSetObjectFloatParameter(client_id_, handle_,
  1004, // (カメラ視野角のID)
  angle, simx_opmode_oneshot);
}
